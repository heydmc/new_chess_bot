...
async def send_message_to_user(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Admin command to send a direct message to a user via their ID."""
    if not await is_admin(update.effective_user.id): 
        return

    if len(context.args) < 2:
        await update.message.reply_text(
            "Usage: `/message <user_id> <your message>`", 
            parse_mode=ParseMode.MARKDOWN_V2
        )
        return
    ...
    try:
        target_user_id = int(context.args[0])
        # Join all arguments after the first one to form the full message
        ...
        if not user_record:
            await update.message.reply_text(
                f"⚠️ User `{target_user_id}` not found in database. They may not have started the bot.",
                parse_mode=ParseMode.MARKDOWN_V2
            )
            return
...
def main() -> None:
    ...
    application.add_handler(CommandHandler("message", send_message_to_user))
